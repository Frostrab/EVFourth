...
import {Button, PopoverIcon, OpenNotification} from '../../../../../components';
...
const Col = Styled.div`
    display:flex
    width :${props => (props.mediaSize === 'pc' || props.mediaSize === 'tablat' ? '30%' : '100%')}
    align-items: center,
    flex-wrap:${props => (props.mediaSize === 'pc' || props.mediaSize === 'tablat' ? 'nowrap' : 'wrap')}
`;
...
const InputRight = Styled.div`
    width :${props => (props.mediaSize === 'pc' || props.mediaSize === 'tablat' ? '70%' : '100%')}
`;
  ...
  componentWillReceiveProps (nextProps) {
    if (nextProps.rowSelect !== this.state.rowSelect) {
      if (nextProps.rowSelect !== this.state.rowSelect) {
        ...
        this.setState ({statusImage: false});
        ...
        if (
          ...
          this.setState ({statusImage: true});
        }
      } else {
        console.log ('else1');
      ...
      if (typeof nextProps.rowSelect.startEvaDateString !== 'undefined') {
        ...
        this.setState ({
          startEvaDateString: startDate[2] +
            '/' +
            startDate[1] +
            '/' +
            startDate[0],
  ...
  render () {
    ...
    const sumTotalScore = () => {
      ...
      if (this.props.weightingKey === 'A2') {
        const totalScore = Math.round (msgTotal / maxScore * 100);
        this.setState ({totalScore: totalScore});
        return totalScore;
      } else {
        this.setState ({totalScore: Math.round (msgTotal)});
        return Math.round (msgTotal);
      }
    };
    ...
    const renderTableHeader = () => {
      return (
        ...
        >
          ...
          <th style={{padding: 10, border: '1px solid black', width: '200px'}}>
            ชื่อตัวชี้วัด
          </th>
          <th style={{padding: 10, border: '1px solid black', width: '40px'}}>
            ระดับคะแนน<i style={{fontSize: 20, color: 'red'}}>*</i>
          </th>
          <th style={{padding: 10, border: '1px solid black', width: '100px'}}>
            หมายเหตุ
          </th>
        </tr>
      );
            ...
            <div style={{width: '100%', display: 'flex', flexWrap: 'wrap'}}>
              <div
                style={{
                  ...
                  marginBottom: 30,
                  marginTop: 30,
                  marginRight: this.props.mediaSize === 'lg' ? '5%' : '0px',
                  width: '800px',
                  borderRadius: '10px',
                  border: '1px solid  #fff',
                }}
              >
                {this.props.mediaSize === 'lg'
                  ? <Row>
                      <Col>
                        <div
                          style={{
              ...
              <div
                style={{
                  ...
                  marginBottom: 30,
                  marginTop: 30,
                  marginLeft: this.props.mediaSize === 'lg' ? 40 : 0,
                  width: '350px',
                  padding: '30px',
                  borderRadius: '10px',
                  border: '1px solid  #fff',
